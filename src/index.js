...
async function run() {
  ...
  app.post("/api/v1/ua/", async function({ body: { ua } }, res) {
    ...
    const promises = [];
    if (browser.name) {
      promises.push(new models.Browser({ name: browser.name }).save());
    }
    if (device.type) {
      promises.push(new models.DeviceType({ name: device.type }).save());
    }
    if (device.vendor) {
      promises.push(new models.DeviceVendor({ name: device.vendor }).save());
    }
    if (os.name) {
      promises.push(new models.OperationSystem({ name: os.name }).save());
    }
  ...
  while (true) {
    ...
    const convertToInt = (dict, value) => {
      ...
      const convertedValue = dicts[dict].get(value);
